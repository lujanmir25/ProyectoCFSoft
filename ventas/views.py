...
#Local
from ventas.models import FacturaEnc, FacturaDet, OrdenFacturaEnc, OrdenFacturaDet
...
#Vista de ordenes de ventas/facuras.
class OrdenFacturaView(LoginRequiredMixin, PermissionRequiredMixin, generic.ListView):
	permission_required = "ventas.view_facturaenc"
...
def orden_facturas(request, id=None):
	...
	if request.method == "POST":
		...
		if not id:
			messages.error(request,'No Puedo Continuar No Pude Detectar No. de Orden de Factura')
			return redirect("ventas:orden_factura_list")
	        
		codigo = request.POST.get("codigo")
		cantidad = request.POST.get("cantidad")
		precio = request.POST.get("precio") #Recordar cambiar la forma de acceder al precio de venta. 
		#s_total = request.POST.get("sub_total_detalle")
		total = request.POST.get("total_detalle")
